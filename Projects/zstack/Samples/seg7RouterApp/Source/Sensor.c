...
void DisplayTwoDigits(unsigned char digit1, unsigned char digit2);
void SendSeg7(unsigned char value);
uint8 SetTwoValues(unsigned char leftDigit, unsigned char rightDigit);
...
{
   P0INP &= ~(0x1<<1);          //P0.1上拉
   P0SEL &= ~((1<<1)|(1<<6)|(1<<7));//P0.1,P0.6,P0.7为GPIO
   P0DIR |= (1<<1)|(1<<6)|(1<<7);   //P0.1,P0.6,P0.7为OUTPUT
   //74HC595初始化
   P0 |= (0x1<<7); //p07 DATA,数据寄存器
   P0 |= (0x1<<6); //p06 CLK，位移寄存器时钟输入
   P0 |= (0x1<<1); //p01 LOCK，数据寄存器开始时钟
}
...
void DisplayTwoDigits(unsigned char digit1, unsigned char digit2)
{
    ...
    // 先发送第二个数码管的数据（级联的第二个74HC595）
    temp = seg7table[digit2];
    for(i = 0; i < 8; i++)
    {
        if(temp & 0x80) {
            P0 |= (0x1<<7); // P0.7值1，DATA
        } else {
            P0 &= ~(0x1<<7); // P0.7值0，DATA
        }
        temp <<= 1;
        P0 &= ~(0x1<<6); // P0.6 CLK下降沿
        P0 |= (0x1<<6);  // P0.6 CLK上升沿（移位）
    }
    
    // 再发送第一个数码管的数据（级联的第一个74HC595）
    temp = seg7table[digit1];
    for(i = 0; i < 8; i++)
    {
        if(temp & 0x80) {
            P0 |= (0x1<<7); // P0.7值1，DATA
    ...
    // 锁存数据，更新显示
    P0 &= ~(0x1<<1); // P0.1 LOCK下降沿
    P0 |= (0x1<<1);  // P0.1 LOCK上升沿（锁存）
}

/****************************************************************
显示单个数字（兼容原有功能，同时显示两个相同数字）
***********************************************************/
void SendSeg7(unsigned char value)
{
    DisplayTwoDigits(value, value);
}

/****************************************************************
设置要显示的两个不同数字
参数: leftDigit - 左边数码管显示的数字(0-15)
      rightDigit - 右边数码管显示的数字(0-15)
返回值: 0表示成功
***********************************************************/
uint8 SetTwoValues(unsigned char leftDigit, unsigned char rightDigit)
{
    // 参数范围保护：确保值在0-15之间
    leftDigit = leftDigit % 16;
    rightDigit = rightDigit % 16;
...
void Init_Sensor(void)
{
  ...
  SHT3X_StartPeriodicMeasurment(REPEATAB_HIGH , FREQUENCY_2HZ);     // work mode
}

void ReadTemp(int* t,int* h)
{
  float temperature;
  float humidity;
