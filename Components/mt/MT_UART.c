  ...
  while (Hal_UART_RxBufLen(port))
  {
    ...
    switch (state)
    {
      ...
      case DATA_STATE:
        ...
        if (bytesInRxBuffer <= LEN_Token - tempDataLen - MT_RPC_POS_CMD1)
        {
        ...
        {
          HalUARTRead (port, &pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen], LEN_Token - tempDataLen- MT_RPC_POS_CMD1);
          tempDataLen += (LEN_Token - tempDataLen - MT_RPC_POS_CMD1);
        }
        /* If number of bytes read is equal to data length, time to move on to FCS */
        if ( (tempDataLen + MT_RPC_POS_CMD1) == LEN_Token )
            state = FCS_STATE;
      ...
      case FCS_STATE:
        ...
        if ((MT_UartCalcFCS (0, (uint8*)&pMsg->msg[0], 1+LEN_Token) == FSC_Token))
        {
